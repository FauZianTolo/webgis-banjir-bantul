...
@section('styles')
    ...
    ══════════════════════════════════════════════════════ */
        ...
        .stat-card:hover {
            ...
            box-shadow: 0 14px 40px rgba(12, 74, 110, 0.14);
        }
        ...
        .stat-card.c-green .stat-num {
            color: #059669;
        }

        .stat-card.c-yellow .stat-num {
            color: #d97706;
        }
        ...
        .stat-card.c-blue .stat-accent {
            background: linear-gradient(90deg, #667eea, #764ba2);
        }

        .stat-card.c-green .stat-accent {
            background: linear-gradient(90deg, #10b981, #059669);
        }

        .stat-card.c-yellow .stat-accent {
            background: linear-gradient(90deg, #f59e0b, #d97706);
        }

        .stat-card.c-red .stat-accent {
            background: linear-gradient(90deg, #ef4444, #dc2626);
        }
    ...
    ══════════════════════════════════════════════════════ */
        ...
        .chart-card:hover {
            box-shadow: 0 14px 48px rgba(8, 145, 178, 0.13);
        }
    ...
    ══════════════════════════════════════════════════════ */
        ...
        .rank-3 {
            background: linear-gradient(135deg, #cd7c3a, #b86a2a);
        }
    ...
    ══════════════════════════════════════════════════════ */
        ...
        .btn-cta-primary:hover {
            ...
            box-shadow: 0 12px 32px rgba(8, 145, 178, 0.42);
            color: white;
        }
        ...
        <div class="container">
            ...
            <div class="row g-3 mb-4">
                <div class="col-6 col-md-3">
                    <div class="stat-card c-blue">
                        ...
                        <div style="min-width:0;">
                            <div class="stat-num" id="stat-total-laporan">{{ number_format($totalLaporan) }}</div>
                ...
                <div class="col-6 col-md-3">
                    <div class="stat-card c-green">
                        ...
                        <div style="min-width:0;">
                            <div class="stat-num" id="stat-total-verified">{{ number_format($totalVerified) }}</div>
                ...
                <div class="col-6 col-md-3">
                    <div class="stat-card c-yellow">
                        ...
                        <div style="min-width:0;">
                            <div class="stat-num" id="stat-total-kecamatan">{{ count($kecamatanStats) }}</div>
                ...
                <div class="col-6 col-md-3">
                    <div class="stat-card c-red">
                        ...
                        <div style="min-width:0;">
                            <div class="stat-num" id="stat-total-terbanyak">{{ $kecamatanStats[0]['total'] ?? 0 }}</div>
            ...
            <div class="info-box mb-4">
                ...
                <div class="info-box-content">
                    ...
                    <p>
                        Data ini menampilkan <strong><span id="info-total-laporan">{{ number_format($totalLaporan) }}</span>
                            kejadian banjir</strong>
                        pada tahun <strong><span id="info-selected-year">{{ $selectedYear }}</span></strong>
                        yang tersebar di <strong><span id="info-total-kecamatan">{{ count($kecamatanStats) }}</span>
                            kecamatan</strong>.
                ...
                <div class="col-12 col-lg-7">
                    <div class="chart-card">
                        ...
                        <div class="source-note">
                            <strong>Sumber:</strong> Data Kejadian Banjir BPBD Kabupaten Bantul (2021–2025).
                        </div>
                        <div class="source-note">
                            <strong>Sumber:</strong> Data Laporan Kejadian Banjir (2026).
                        </div>
                    </div>
                </div>
                    ...
                    <div class="chart-card">
                        <div class="chart-card-title">
                            ...
                            <span style="font-size:13px;font-weight:600;color:#64748b;">(<span
                                    id="donut-year">{{ $selectedYear }}</span>)</span>
            ...
            <div class="chart-card mb-4">
                <div class="chart-card-title">
                    ...
                    <span style="font-size:13px;font-weight:600;color:#64748b;">(<span
                            id="table-year">{{ $selectedYear }}</span>)</span>
                </div>
                <div class="table-responsive">
                    <table class="stats-table table table-hover w-100">
                        ...
                        <tbody id="kecamatan-detail-tbody">
...
@section('script')
    ...
    <script>
        ...
        const kecamatanByYear = @json($kecamatanByYear ?? []);
        const verifiedByYear = @json($verifiedByYear ?? []);
        let yearlyData = @json($yearlyData);
        let selectedYear = Number({{ $selectedYear }});
        let trendChart = null;
        let donutChart = null;
        ...
        function getKecamatanForYear(year) {
            const yearData = kecamatanByYear[String(year)] || kecamatanByYear[year] || {};
        ...
        function getTotalForYear(year) {
            const monthly = yearlyData[String(year)] || yearlyData[year] || {};
            if (Array.isArray(monthly)) {
                return monthly.reduce((sum, val) => sum + Number(val || 0), 0);
            }
            return Object.values(monthly).reduce((sum, val) => sum + Number(val || 0), 0);
        }

        function getStatusByPercent(pct) {
            if (pct >= 10) return {
                text: 'Tinggi',
                cls: 's-high',
            ...
            return {
                text: 'Rendah',
                cls: 's-low',
                fill: 'linear-gradient(90deg,#3b82f6,#2563eb)'
            };
        }

        function updateSummaryCards(year, kecData) {
            const totalKejadian = getTotalForYear(year);
            const totalVerified = Number((verifiedByYear[String(year)] ?? verifiedByYear[year]) || 0);
            const totalKecamatan = kecData.length;
            const titikTerbanyak = kecData[0]?.total || 0;

            document.getElementById('stat-total-laporan').textContent = numberFormat(totalKejadian);
            document.getElementById('stat-total-verified').textContent = numberFormat(totalVerified);
            document.getElementById('stat-total-kecamatan').textContent = numberFormat(totalKecamatan);
            document.getElementById('stat-total-terbanyak').textContent = numberFormat(titikTerbanyak);

            document.getElementById('info-total-laporan').textContent = numberFormat(totalKejadian);
            document.getElementById('info-total-kecamatan').textContent = numberFormat(totalKecamatan);
            document.getElementById('info-selected-year').textContent = year;
        ...
        function updateKecamatanTable(data) {
            ...
            if (!data.length) {
                tbody.innerHTML =
                    '<tr><td colspan="5" class="text-center text-muted py-4">Tidak ada data pada tahun ini</td></tr>';
                return;
            }

            const totalAll = data.reduce((s, d) => s + Number(d.total || 0), 0);
            tbody.innerHTML = data.map((item, idx) => {
                const pct = totalAll > 0 ? ((Number(item.total) / totalAll) * 100) : 0;
                const rankClass = idx === 0 ? 'rank-1' : (idx === 1 ? 'rank-2' : (idx === 2 ? 'rank-3' : 'rank-n'));
                const status = getStatusByPercent(pct);
            ...
            <td style="min-width:130px;">
                <div class="prog-wrap">
                    <div class="prog-fill" style="width:${pct}%;background:${status.fill};">
                        ${pct.toFixed(1)}%
                    </div>
                </div>
            </td>
            <td class="text-center">
                <span class="s-badge ${status.cls}">${status.text}</span>
            </td>
        </tr>`;
            }).join('');
        }

        function updateDonutLegend(top10, total) {
            const legend = document.getElementById('donutLegend');
            if (!legend) return;

            if (!top10.length) {
                legend.innerHTML = '<div class="text-muted small">Tidak ada data pada tahun ini.</div>';
                return;
            }

            legend.innerHTML = top10.map((k, i) => {
                const pct = total > 0 ? ((Number(k.total) / total) * 100).toFixed(1) : '0.0';
            ...
            <span class="donut-legend-val">${numberFormat(k.total)}</span>
        ...
        function renderDonutChart(data) {
            ...
            const total = top10.reduce((s, k) => s + Number(k.total || 0), 0);
            const labels = top10.map(k => k.kecamatan);
            const vals = top10.map(k => Number(k.total || 0));
            ...
            const center = document.querySelector('.dc-num');
            if (center) center.textContent = numberFormat(total);
            updateDonutLegend(top10, total);
        }
        ...
        function changeYear(year) {
            ...
            document.querySelectorAll('.year-pill').forEach(btn => {
                btn.classList.toggle('active', Number(btn.dataset.year) === selectedYear);
            });

            const yearKecData = getKecamatanForYear(selectedYear);

            initTrendChart(selectedYear);
            renderDonutChart(yearKecData);
            updateKecamatanTable(yearKecData);
            updateSummaryCards(selectedYear, yearKecData);
        }
